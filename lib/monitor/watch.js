...
function watch() {
  ...
  const promise = new Promise(function (resolve) {
    ...
    var fileLoadingWarningTimerFiredCount = 0;
    var fileLoadingWarning = function () {
      fileLoadingWarningTimerFiredCount += 1;
      utils.log.info('Warning: not yet monitoring files for changes');
      setTimeout(fileLoadingWarning, 5000);
    }
    var fileLoadingWarningTimer = setTimeout(fileLoadingWarning, 3000);
    ...
    watcher.on('ready', function () {
      clearTimeout(fileLoadingWarningTimer);
      if (fileLoadingWarningTimerFiredCount > 0) {
        utils.log.info('Now monitoring files for changes');
      }
      watcher.ready = true;
      resolve(total);
